...
\def\geommarkinline#1#2#3{%
  % Disabled: Using line-level marking instead of paragraph start/end markers
  % \ifvmode
...
\else
  \directlua{
    ...
    function geom_write_line_data(id, line, w, h, d)
        ...
        local json = '{"id":"' .. id .. '","role":"P","type":"line","line":' .. line .. ',"xsp":"' .. x .. '","ysp":"' .. y .. '","page":' .. page .. ',"w":' .. w .. ',"h":' .. h .. ',"d":' .. d .. '}' .. string.char(10)
  ...
  \AtBeginDocument{%
    \directlua{
        if not geom_linebreak_callback then
            texio.write_nl("ERROR: geom_linebreak_callback not defined!")
        elseif not luatexbase then
            texio.write_nl("GEOM: luatexbase not available")
        elseif not luatexbase.add_to_callback then
            texio.write_nl("GEOM: luatexbase.add_to_callback not available")
        else
            local success, err = pcall(function()
                luatexbase.add_to_callback("post_linebreak_filter", geom_linebreak_callback, "geom_lines")
            end)
            if success then
                texio.write_nl("GEOM: Line-level marking enabled")
            else
                texio.write_nl("GEOM: Callback registration failed: " .. tostring(err))
            end
        end
    }%
  }%
  \def\geommarkpara#1{%
    \directlua{geom_current_para_id = "#1"}%
  }%
\fi