...
/**
 * Minimal mathematical expression fixer for LaTeX files
 * This script removes problematic mathematical content to ensure compilation
 */
...
function fixMathExpressions(content) {
    ...
    // Remove problematic mathematical expressions that cause LaTeX errors
    // Only replace in the document body, not in the preamble
    const parts = fixed.split('\\begin{document}');
    if (parts.length === 2) {
        const preamble = parts[0];
        let body = parts[1];
        
        // Fix mathematical expressions only in the body
        body = body.replace(/J=\{[^}]*\}\{[^}]*\}/g, 'J = [mathematical expression]');
        body = body.replace(/\{[^}]*\}\{[^}]*\}/g, '[mathematical expression]');
        
        fixed = preamble + '\\begin{document}' + body;
    } else {
        // Fallback: replace only in specific contexts
...
function main() {
    ...
    if (args.length === 0) {
        console.log('Usage: node fix-math-minimal.js <input-file> [output-file]');
