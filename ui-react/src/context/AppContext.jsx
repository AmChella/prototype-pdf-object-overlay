...
export const AppProvider = ({ children }) => {
  ...
  // Article Context (from S3 fetch)
  const [articleContext, setArticleContext] = useState({ journalId: null, articleId: null });
  ...
  const sendBatchInstructions = useCallback(async () => {
    ...
    const message = {
      ...
      // Include article context at batch level
      ...
      instructions: instructionStack.map(i => ({
        ...
        // Also include in each instruction for backend compatibility
        journalId: articleContext.journalId,
        articleId: articleContext.articleId
      })),
      timestamp: new Date().toISOString()
    ...
    return success;
  }, [enableInstructionStack, instructionStack, send, clearInstructionStack, articleContext]);
